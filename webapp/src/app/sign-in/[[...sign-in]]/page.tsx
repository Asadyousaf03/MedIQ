    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">Use one of the local demo accounts or your patient account.</p>
        </div>
        ...
        <div className="card-surface p-8">
          ...
          <form action="/api/auth/sign-in" method="post" className="space-y-4">
            <div>
              ...
              <input id="email" name="email" type="email" required className="input-field" placeholder="doctor@mediq.local" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="password">Password</label>
              <input id="password" name="password" type="password" required className="input-field" placeholder="Enter your password" />
            </div>
            <button type="submit" className="btn-primary w-full">Sign in</button>
          </form>

          <div className="mt-6 text-center text-sm text-slate-500">
            Need an account? <Link href="/sign-up" className="font-medium text-[var(--primary)] hover:text-[var(--primary-hover)]">Create one</Link>
          </div>
        </div>
      </div>
    </div>
